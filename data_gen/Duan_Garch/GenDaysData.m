function dataset = GenDaysData(max_day, day_num, r, T, m, alpha, beta, omega, theta, lambda)
    ...
    % - m: Moneyness factors
    % - alpha, beta, omega, theta, lambda: GARCH model parameters
    % Outputs:
    ...
    [S, h0] = mcDuan(M, N, S_init, Z, r, omega, alpha, beta, theta, lambda);
    ...
    for i = 1:T_len
        for j = 1:m_len
            ...
            [V_C, ~] = datagen(t(i), r, S0, h0, K, alpha, beta, omega, theta,lambda, 1); % Call
            [V_P, ~] = datagen(t(i), r, K, h0, S0, alpha, beta, omega, theta, lambda, -1); % Put
            ...
            dataset(:, idx) = [S0; K/S0; r; t(i); 1; alpha; beta; omega; theta; lambda; V_C];
            idx = idx + 1;

            % Store put option data
            dataset(:, idx) = [S0; K/S0; r; t(i); -1; alpha; beta; omega; theta; lambda; V_P];
            idx = idx + 1;
        end
    end
end
