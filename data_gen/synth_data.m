function [A_sig, A_prices, S0] = synth_data(maturity, r, S0, N, M, h0, alpha, beta, omega, gamma, lambda)
    % Parameters:
    ...
    %   S0 - Initial prices used for each maturity.
    ...
    strike_prices = linspace(0.8, 1.2, 8)*S0;
    A_sig = zeros(maturity, 1);
    A_prices = zeros(maturity, 1);

    % Prepare initial prices for each maturity
    S0 = S(mod(0:(maturity - 1), path_days) + 1);

    parfor j = 1:maturity % iterate through the maturities in parallel
        [A_sig(j), A_prices(j), ~] = impVol_HN(r, lambda_Q, omega_Q, rho, alpha_Q, gamma_Q, h0, S0(j), strike_prices(j), j, N, m_h, m_x, -1);
    end
end