function dataset = GenDaysData(max_day, day_num, r, T, m, alpha, beta, omega, gamma, lambda)
    ...
    [S, h0] = mcGJR(M, N, S_init, Z, r, omega, alpha, beta,  lambda);
    ...
    for i = 1:T_len
        for j = 1:m_len
            ...
            [V_C, ~] = datagen(t(i), r, S0, h0, K, alpha, beta, omega, lambda, 1); % Call
            [V_P, ~] = datagen(t(i), r, K, h0, S0, alpha, beta, omega,  lambda, -1); % Put
